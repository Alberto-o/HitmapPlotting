...
SCT_CalibHitmapSvc::book(){
  ...
  for( ; waferItr not_eq m_waferItrEnd; ++waferItr ) {
    ...
    std::string name2D=hitmapPaths[bec2Index(m_pSCTHelper->barrel_ec( waferId ))] + formattedPosition + "2D";
    TH1F* hitmapHisto_tmp = new TH1F( TString( formattedPosition ), TString( histotitle ), nbins, firstStrip-0.5, lastStrip+0.5 );
    //    TH2F* hitmapHisto_tmp2D = new TH2F( TString( formattedPosition2D ), TString( histotitle ), nbins, firstStrip-0.5, lastStrip+0.5 ,10,5000,105000);
    TH2F* hitmapHisto_tmp2D = new TH2F( TString( formattedPosition2D ), TString( histotitle ), nbins, firstStrip-0.5, lastStrip+0.5 ,1,0,0);
    //    hitmapHisto_tmp2D->SetCanExtend(kTRUE);
    hitmapHisto_tmp2D->GetYaxis()->SetCanExtend(kTRUE);
    ...
    if( m_thistSvc->regHist( name.c_str(), hitmapHisto_tmp ).isFailure()) {
      ...
      //      m_phistoVector2D.push_back( hitmapHisto_tmp2D );
    }
    cout<<"2D"<<endl;
    cout<<name2D.c_str()<<endl;
    if(m_thistSvc->regHist( name2D.c_str(), hitmapHisto_tmp2D ).isFailure() ) {
...
SCT_CalibHitmapSvc::read(const std::string & fileName){
  ...
  for( ; waferItr not_eq m_waferItrEnd; ++waferItr ) {
    ...
    TH2F* hitmapHisto_tmp2D = (TH2F*) fileHitmap->Get(name2D.c_str());
...
SCT_CalibHitmapSvc::fill(const bool fromData){
  ...
  for( int i = 0; i != MaxEntry; ++i ) {
    ...
    for( int strip(theFirstStrip); strip !=endStrip; ++strip) {
      ...
      pThisHisto2D->Fill( strip, eventNumber);
    }
  }
  return result;
}
...
// bool
// SCT_CalibHitmapSvc::binHistograms(int eventsPerWindow){
//   bool result(true);
  ...
  //   (*it)->SetBins(nbins, -0.5, 768+0.5 ,yAxisBins,0,totalEventNumber);
  // }
